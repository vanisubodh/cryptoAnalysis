  ...
  def correlateTweetsAndCurrencyReturns(
    ...
    val tweetsVsCryptoPrices =
      userTweets
        ...
        .withColumn(
          "daily_return",
          abs(log($"next_day_close" / $"prev_day_close"))
        )

    tweetsVsCryptoPrices
      .select(
        "date",
        "retweet_count",
        "daily_return",
    ...
    val rddX = tweetsVsCryptoPrices.select($"retweet_count_double").coalesce(1).rdd.map(_.getDouble(0))
    val rddY = tweetsVsCryptoPrices.select($"daily_return").coalesce(1).rdd.map(_.getDouble(0))
